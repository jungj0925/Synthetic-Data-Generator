...
def run_video_animation(drive_root, fname, network_pkl, grid_dims, seeds, outdir, fov_deg, truncation_psi, truncation_cutoff, reload_modules, lms_cond, fixed_camera, num_frames):
    ...
    for k, img_path in tqdm(enumerate([img for img in img_list])):
        ...
        for idx, seed in enumerate(seeds):
            ...
            camera_params = (np.array(label_list[k-1][1]) + np.array(label_list[k][1])) / 2 # add camera smoothness
            ...
            imgs.append(img)
            individual_img_path = os.path.join(img_dir, f'{img_id}_seed{seed}.png')
            individual_img = (img.clamp(-1, 1) + 1) * 127.5
            imageio.imwrite(individual_img_path, individual_img.byte().cpu().numpy().transpose(1, 2, 0))
